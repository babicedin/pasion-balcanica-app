...
/// Shop detail — visual twin of [FoodDetailScreen]: hero with swipeable
/// photos, branded category pill, directions + call CTAs, gallery, videos.
...
class _ShopDetailScreenState extends ConsumerState<ShopDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final category = (categoriesAsync.asData?.value ?? const <CategoryInfo>[])
            .firstWhere(
          (c) => c.slug == shop.categorySlug,
          orElse: () => CategoryInfo.unknown(shop.categorySlug),
        );

    final heroUrls = shop.imageUrls.isNotEmpty
        ? shop.imageUrls
        : <String>[shop.heroImageUrl];
          ...
          children: [
            Column(
              children: [
                ...
                Expanded(
                  child: _Info(shop: shop, locale: locale, s: s),
                ),
              ],
            ),
              ...
              child: Row(
                ...
                children: [
                  ...
                  _CircleGlass(
                    ...
                    onTap: () {
                      final title = shop.titleFor(locale);
                      final kicker = shop.kickerFor(locale);
                      final parts = <String>[
                        title,
...
class _Info extends StatelessWidget {
  ...
  final String locale;
  final S s;
  const _Info({required this.shop, required this.locale, required this.s});

  @override
  Widget build(BuildContext context) {
    final pb = context.pb;
    final hasPhone = shop.phone != null && shop.phone!.trim().isNotEmpty;
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              if (shop.kickerFor(locale).isNotEmpty) ...[
                Kicker(shop.kickerFor(locale).toUpperCase()),
                const SizedBox(height: 6),
              ],
              Text(
                shop.titleFor(locale),
              ...
              Row(
                children: [
                    ...
                    onTap: () {
                      if (shop.latitude != null && shop.longitude != null) {
                        Launchers.directions(
                          lat: shop.latitude!,
                          lng: shop.longitude!,
                          label: shop.titleFor(locale),
                        );
                      } else if ((shop.address ?? '').trim().isNotEmpty) {
                        Launchers.directionsToAddress(shop.address!);
                      }
                    },
                  ),
                  ...
                  ActionBtn(
                    ...
                    label: s.call,
                    onTap: () {
                      if (hasPhone) Launchers.call(shop.phone!);
                    },
                  ),
                ],
              ),
              ...
              if (shop.latitude != null && shop.longitude != null) ...[
                ...
                Text(
                  _fmt(shop.latitude!, shop.longitude!),
                  style: TextStyle(
              ...
              if (shop.imageUrls.length > 1) ...[
                ...
                _Gallery(urls: shop.imageUrls),
              ],

              if (shop.youtubeUrls.isNotEmpty) ...[
                const SizedBox(height: 22),
                SectionHeader(s.videos.toUpperCase()),
                _VideoList(urls: shop.youtubeUrls),
              ],
            ],
          ),
        ),
