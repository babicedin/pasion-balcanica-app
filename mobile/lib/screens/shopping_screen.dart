...
/// Shopping browser — visual twin of [FoodScreen]: dynamic category chips
/// (driven by `shop_categories`) + a 2-up grid of cards.
///
...
class _ShoppingScreenState extends ConsumerState<ShoppingScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final s = S.of(locale);
    final async = ref.watch(shopsProvider);
    final categoriesAsync = ref.watch(shopCategoriesProvider);
    final all = async.asData?.value ?? const <Shop>[];
    final allCategories =
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Kicker(s.shopAndBrowse),
                  const SizedBox(height: 6),
                  AccentHeadline(
                    pre: s.whereLocalsShopPre,
                    accent: s.whereLocalsShopAccent,
                  ),
                ],
              ),
            ),
              ...
              sliver: SliverGrid.builder(
                ...
                itemBuilder: (_, i) => _ShopCard(
                  ...
                  onTap: () => Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (_) => ShopDetailScreen(shop: items[i]),
                    ),
                  ),
                ),
              ),
                      ...
                      children: [
                        if (shop.imageUrls.isNotEmpty)
                          MediaBadge(
                            ...
                            count: shop.imageUrls.length,
                          ),
                        if (shop.imageUrls.isNotEmpty && shop.videoCount > 0)
                          const SizedBox(width: 10),
                        if (shop.videoCount > 0)
                          MediaBadge(
                            icon: Icons.play_arrow_rounded,
                            count: shop.videoCount,
                          ),
                      ],
                    ),
                  ],
