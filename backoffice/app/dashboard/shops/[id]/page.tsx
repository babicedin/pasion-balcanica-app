...
export default function EditShopPage() {
  ...
  const [record, setRecord] = useState<ShopRow | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setRecord(data as ShopRow | null);
      setLoading(false);
    }
    load();
    return () => {
  ...
  return (
    <BilingualProvider initialValues={initial}>
      <EditShopForm record={record} router={router} />
    </BilingualProvider>
  );
}
...
function EditShopForm({
  ...
  const [error, setError] = useState<string | null>(null);
  const [category, setCategory] = useState(record.category ?? "souvenirs");
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const supabase = createSupabaseBrowserClient();
    const { error } = await supabase.from("shops").update(payload).eq("id", record.id);
  ...
  async function onDelete() {
    ...
    const supabase = createSupabaseBrowserClient();
    const { error } = await supabase.from("shops").delete().eq("id", record.id);
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        eyebrow="Content"
        title={`Edit shop: ${record.title_en}`}
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <div className="card p-6 md:p-7 space-y-5">
          ...
          <p className="text-xs text-muted">
            Order is managed by drag-and-drop on the shopping list.
          </p>
        </div>
